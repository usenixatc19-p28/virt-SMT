...
#ebizzy
bench="ebizzy"
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_ebizzy/vm2.p.$bench  &
for i in 1 2 3 4 5
do
...
bench="hack"
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_hack/vm2.p.$bench  &
for i in 1 2 3 4 5
do
...
bench="kern"
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_kern/vm2.p.$bench  &
for i in 1 2 3 4 5
do
...
bench="dbench"
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_dbench/vm2.p.$bench  &
#mount disk first
for i in 1 2 3 4 5
do
...
bench="pbzip2"
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_pbzip2/vm2.p.$bench  &
#compress
for i in 1 2 3 4 5
do
...
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_sockperf/vm2.sockperf  &
ssh $kvm2@$ip2 "time taskset -c 0-22 $SOCKPERF_BIN server --tcp -i $ip2 -p 12345" &>> $ctx_sockperf/vm2.sockperf &
for i in 1 2 3 4 5
do
	ssh $kvm1@$ip1 "time taskset -c 0-22 $SOCKPERF_BIN pp --tcp -i 192.168.122.98 -p 12345 -t 30" &>> $ctx_sockperf/vm1.sockperf
	#sleep 60
done
...
#sysbench mysql OLTP
ssh $kvm2@$ip2 "/home/$kvm2/parsec-3.0/parsec/vPair/inVM/accurate_cpu" >> $ctx_oltp/vm2.oltp  &
for i in 1 2 3 4 5
do
