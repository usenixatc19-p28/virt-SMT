 ...
 * perf_lbound.c
 *
 * System performance lower-bound:
 * two hyperthreads on one physical core running spin-lock, Or, 
 * two hyperthreads on one physical core running plus-1. 
 *
 */
#define _GNU_SOURCE
...
uint64_t plus_start = 0;
uint64_t mwait_start = 24;
uint64_t flag = 0;
...
void *_thread_func(void *arg) {
	...
	vn = get_affinity();
	printf("CPU plus 1 worker is on %lu\n", vn);
	int pid = syscall(SYS_gettid);
	printf("CPU plus 1 worker thread PID number is %d\n", pid);
	//set_idle_priority();
...
void *mwait_func(void *arg) {
	uint64_t vn = *((uint64_t *) arg);
	set_affinity(vn);

	vn = get_affinity();
	printf("mwait worker is on %lu\n", vn);
	int pid = syscall(SYS_gettid);
	printf("mwait worker thread PID number is %d\n", pid);
	//set_idle_priority();
	//set_nice_priority(20, pid);
	while (1) {
		if (flag == 1) break;
	...
	pthread_spin_lock(&(t[vn]));
	//printf("Spinlock error on vCPU %lu!\n", vn);
	pthread_spin_unlock(&(t[vn]));
}
...
void init_probe_thread(void) {
	...
	t = (pthread_spinlock_t *) malloc(sizeof(pthread_spinlock_t) * __vcpu_num);
	if (t == NULL)
		handle_error("malloc error!");
	...
	for (i = start_vcpu - 1, j = 24; i < end_vcpu + 1; i++, j++) {
	//XXX vCPU 0 is dedicated to handle all the interrupts
		_vcpu[i].vcpu_num = i;
		_vcpu[j].vcpu_num = j;
		_vcpu[i].plus_one = 0;
		if (i == 0) {
			pthread_spin_init(&(t[j]), 0);
			ret = pthread_create(&(_p[i]), NULL, _thread_func, &(_vcpu[i].vcpu_num));
			if (ret != 0) {
				printf("Pthread create error!\n");
				exit(EXIT_SUCCESS);
			}
			ret = pthread_create(&(_p[j]), NULL, mwait_func, &(_vcpu[j].vcpu_num));
			if (ret != 0) {
				printf("Pthread create error!\n");
				exit(EXIT_SUCCESS);
			}
		...
		if (i >= 1 && i <= 11) {
			...
			ret = pthread_create(&(_p[j]), NULL, _thread_func, &(_vcpu[j].vcpu_num));
			if (ret != 0) {
				printf("Pthread create error!\n");
				exit(EXIT_SUCCESS);
			}
		...
		if (i >= 12 && i <= 22) {
			pthread_spin_init(&(t[i]), 0);
			pthread_spin_init(&(t[j]), 0);
			ret = pthread_create(&(_p[i]), NULL, mwait_func, &(_vcpu[i].vcpu_num));
			if (ret != 0) {
				printf("Pthread create error!\n");
				exit(EXIT_SUCCESS);
			}
...
int main(int argc, char **argv) {
	...
	set_pid_affinity(23, pid); //main thread is used for timer
	printf("Main thread's PID number is %d\n", pid);
	printf("Main thread is on vCPU %lu\n", get_pid_affinity(pid));

	init_probe_thread();

	pthread_spin_lock(&(t[24]));
	for (j = 12; j <= 22; j++) {
		i = j + 24;
		pthread_spin_lock(&(t[j]));
		pthread_spin_lock(&(t[i]));
	}
	sleep(10);
	flag = 1;
	uint64_t start = debug_time_monotonic_usec();
	while (1) {
		//printf("time diff is: %lu\n", (debug_time_monotonic_usec() - start));
	    if (debug_time_monotonic_usec() - start >= 30000000) {
		    flag = 0;
			break;
		}
	}
	...
	for (i = 1; i <= 11; i++) {
		j = i + 24;
	    //printf("vCPU[%d]'s speed is: %lu ops\n", i, (_vcpu[i].plus_one/30));
	    printf("%lu\n", ((_vcpu[i].plus_one)/30));
	    printf("%lu\n", ((_vcpu[j].plus_one)/30));
	}
	pthread_spin_unlock(&(t[24]));
	for (j = 12; j <= 22; j++) {
		i = j + 24;
		pthread_spin_unlock(&(t[j]));
		pthread_spin_unlock(&(t[i]));
	}
